...
- L：Context Pack → A1 → A2 → A4 → A5a → A8（Dashboard / 報表 / KPI / 統計頁 / 客戶列表 / 工作管理頁 / 狀態監控頁 / 老闆會閱讀的介面）→ commit。Gate 3/4 觸發時 A6/A7 中斷流程
- XL：A0 → Context Pack → A1 → A2 → A3 → A4 → A5a → A5b → A8（Dashboard / 報表 / KPI / 統計頁 / 客戶列表 / 工作管理頁 / 狀態監控頁 / 老闆會閱讀的介面）→ commit。Gate 3/4 觸發時 A6/A7 中斷流程。A9 由使用者決定
...
| A3 UI 審查 | 美感、一致性、資訊密度 | 三階段：(1)審查→強制調用`impeccable:impeccable`(critique+audit)，只報Critical/High，格式`[元件]在[頁面][問題]，建議[修正]`(附檔案:行號、影響範圍)，不得僅以文字描述；(2)修復→新建用`document-skills:frontend-design`，大幅改造用`impeccable:impeccable`(craft/shape)；(3)打磨→`make-interfaces-feel-better`逐項查16條微互動原則。`design-an-interface`屬API/程式介面設計，不在此處使用 | 擴大功能、改業務邏輯 |
| A4 架構審查 | data flow、service boundary、DB結構、模組耦合 | 建議架構邊界，無上層衝突時可自行決定。需求不清時呼叫A0 | 建議K8s/微服務/新語言/新DB/訊息佇列 |
| A5a 流程審查 | 流程合理性、使用者體驗 | L+觸發，A2/A4後執行。逐項：(1)誰會用到 (2)使用頻率 (3)步驟數（>5警示）(4)認知負擔 (5)更簡單方法。輸出：流程風險/簡化建議/通過與否 | 決定部署修法、改業務決策 |
