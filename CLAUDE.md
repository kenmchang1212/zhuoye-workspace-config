...
| A3 UI 審查 | 美感、一致性、資訊密度 | 只報Critical/High。格式：`[元件] 在 [頁面] [問題描述]，建議 [修正]`（附檔案:行號、影響範圍）。強制調用 `impeccable:impeccable` 執行審查（必要）；修復階段依問題類型選用 `document-skills:frontend-design`（重建頁面/元件）、`design-an-interface`（單一介面設計）、`make-interfaces-feel-better`（互動優化）。不得僅以文字描述執行審查 | 擴大功能、改業務邏輯 |
